...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var cutView: UIImageView!
    ...
    func grabCutFaces(){ //GrabCut function takes face rectangles generated by rectanglesRequest and cuts out the faces
        ...
        for rect in _rectangles{
            print(rect)
            let m = gb.doGrabcut(self._transform, foregroundBound: rect)
            if let f = gb.masking(self._transform, mask: m){
                _faces.append(f)
            }
        }
        if (!_faces.isEmpty){
            DispatchQueue.main.async {
                ...
                self.imageView.image = self.drawRectangleOnImage(image: self._transform, rectangle: self.convRect(rect: rect))
                self.cutView.image = self._faces[0]
            }
        }
        DispatchQueue.main.async {
    ...
    func convRect(rect: CGRect) -> CGRect{
        return CGRect(x: rect.origin.x*self._image.size.width-0.125*rect.width*self._image.size.width, y: rect.origin.y*self._image.size.height-rect.height*self._image.size.height, width: 1.25*rect.width*self._image.size.width, height: 2*rect.height*self._image.size.height)
    }
    ...
    func handleRectangles(request: VNRequest, error: Error?) {
        ...
        for d in observations{
            _rectangles.append(convRect(rect: d.boundingBox))
        }
    ...
    func predict(pixelBuffer: CVPixelBuffer) {
        ...
        if let boundingBoxes = try? yolo.predict(image: resizedPixelBuffer) {
            ...
            for box in boundingBoxes{
                ...
                _rectangles.append(box.rect)

            }
            
            self.imageView.image = _transform
        }
